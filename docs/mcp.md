...
- `find`: discover kernel functions, tracepoints, loaded BPF programs, BTF
  types, and kallsyms symbols;
...
Filter and capture expressions may reference a typed return value as
`$retval`, with the same required explicit concrete cast as the CLI, for
example `"(int)$retval < 0"`. MCP automatically selects exit tracing, or paired
entry/exit tracing when duration, function-graph, or instruction capture needs
the entry event. To apply a pcap filter to a packet-typed return value, set
`filter.packet_source` to `"retval"`; to decode that returned packet, set
`capture.packet` to `true` and `capture.packet_source` to `"retval"`. Both
packet sources default to `"argument"`. The legacy `(r)` prefix in a packet
filter remains accepted. kprobe.multi does not support typed `$retval`
expressions or retval packet sources.
...
Agents must present every trace result for human review rather than repeat raw
JSON. Present ordinary events as a short chronological list with the target,
process, CPU, timestamp, selected values, return value, and duration. Present
`function_graph` events as an indented call tree, including the root call's
...
omitting it searches all five classes. Start with just `pattern` to discover
all available information, and specify `kind` only to narrow the search.
Results are capped at 50 by default and 200 at most. The `total` and
`truncated` fields help agents refine broad queries.
...
kprobe.multi. The function search also covers text symbols found only in
`/proc/kallsyms`; these results report `btf: false`, omit BTF-only prototype
fields, and may still be traceable through kprobe.multi. Functions that are not
traceable are still returned.
...
For example, `find({"pattern":"__per_cpu_offset"})` returns the kallsyms
symbol and any matching BTF metadata. `ksym` results include an exact
hexadecimal `address`, the `symbol_type` letter from `/proc/kallsyms`, and the
providing module. They include data symbols as well as text symbols and retain
distinct addresses for duplicate names. Use the address with an explicit type
in `read`. BTF metadata is returned separately when available; an empty
`btf_type` search does not mean a kallsyms symbol is absent.
