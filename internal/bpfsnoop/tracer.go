...
func prepareTraceFlags(options TraceOptions) (*Flags, error) {
	...
	pktFilter.retval = pktFilter.retval || options.PacketFilterRetval
	...
	outputPktRetval = options.Capture.PacketRetval

	needsRetval := options.Capture.Retval || pktFilter.retval || outputPktRetval
	for _, filter := range argFilter.args {
		needsRetval = needsRetval || filter.retval
	}
	for _, output := range argOutput.args {
		needsRetval = needsRetval || slices.Contains(output.vars, cc.RetvalName)
	}
	modes = []string{TracingModeEntry}
	if needsRetval {
		modes = []string{TracingModeExit}
	}
	if options.Capture.Duration || options.Capture.FunctionGraph ||
		(options.Capture.Instructions && needsRetval) {
