...
// TraceSelectedArgument is one captured argument expression and the metadata
// needed to decode it at the API boundary.
type TraceSelectedArgument struct {
	Raw ReadKernelResult
}
	...
	for i := range info.args {
		...
		selected = append(selected, TraceSelectedArgument{
			Raw: ReadKernelResult{
				...
				BTFType:        arg.t,
				Buffer:         slices.Clone(data[offset : offset+arg.size]),
				Size:           arg.size,
