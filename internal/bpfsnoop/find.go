...
func findKernelSymbolInfo(name string, ksyms *Kallsyms) (FindKernelFunctionInfo, bool) {
	...
	info.KprobeMultiEntry = !skipKprobeMultiSymbol(name)
	info.KprobeMultiExit = info.KprobeMultiEntry && !slices.Contains(noreturnFuncs, name)
	return info, true
}

// FindKernelSymbol returns kallsyms metadata for one kernel text symbol.
func FindKernelSymbol(name string, ksyms *Kallsyms) (FindKernelFunctionInfo, bool) {
	return findKernelSymbolInfo(name, ksyms)
}

// FindKernelSymbolNames iterates kallsyms text symbol names in lexical order.
func FindKernelSymbolNames(ksyms *Kallsyms) iter.Seq[string] {
	return func(yield func(string) bool) {
		names := slices.Sorted(maps.Keys(ksyms.n2s))
...
func FindKernelFunction(fn *btf.Func, ksyms *Kallsyms) FindKernelFunctionInfo {
	info, ok := findKernelSymbolInfo(fn.Name, ksyms)
	if !ok {
		return info
	}
	...
	if !info.Traceable {
		info.KprobeMultiEntry = false
		info.KprobeMultiExit = false
		return info
	}
