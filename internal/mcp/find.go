...
func compareFindMatch(a, b findMatch) int {
	...
	if cmp := strings.Compare(a.SymbolType, b.SymbolType); cmp != 0 {
		return cmp
	}
...
func (c *findCollector) add(match findMatch) {
	key := fmt.Sprintf("%s\x00%s\x00%s\x00%d\x00%s\x00%s\x00%x\x00%s",
		match.Kind, match.Name, match.Module, match.ID, match.ProgramName, match.BTFKind, match.Address, match.SymbolType)
...
func validateFindOptions(options *FindOptions) error {
	...
	switch options.Kind {
	case "", FindKindFunction, FindKindTracepoint, FindKindBPFProgram, FindKindBTFType, FindKindKsym:
	default:
...
func populateBTFDetails(match *findMatch, typ btf.Type) {
	details := btfx.DescribeType(typ)
	match.BTFKind = details.Kind
	match.BTFSize = details.Size
	match.BTFReturnType = details.ReturnType
	match.BTFTargetType = details.TargetType
	match.BTFElementType = details.ElementType
	match.BTFIndexType = details.IndexType
	match.BTFElementCount = details.ElementCount
	match.BTFEncoding = details.Encoding
	match.BTFEnumSigned = details.EnumSigned
	match.BTFLinkage = details.Linkage
	match.BTFFwdKind = details.ForwardKind
	match.BTFTags = details.Tags

	if details.Members != nil {
		match.BTFMembers = make([]FindBTFMember, 0, len(details.Members))
		for _, member := range details.Members {
			match.BTFMembers = append(match.BTFMembers, FindBTFMember(member))
		}
	}
	if details.Arguments != nil {
		match.BTFArguments = make([]FindBTFArgument, 0, len(details.Arguments))
		for _, argument := range details.Arguments {
			match.BTFArguments = append(match.BTFArguments, FindBTFArgument(argument))
		}
	}
	if details.EnumValues != nil {
		match.BTFEnumValues = make([]FindBTFEnumValue, 0, len(details.EnumValues))
		for _, value := range details.EnumValues {
			match.BTFEnumValues = append(match.BTFEnumValues, FindBTFEnumValue(value))
		}
	}
	if details.Variables != nil {
		match.BTFVariables = make([]FindBTFVariable, 0, len(details.Variables))
		for _, variable := range details.Variables {
			match.BTFVariables = append(match.BTFVariables, FindBTFVariable(variable))
		}
	}
}
...
func newBTFTypeMatch(name, module string, typ btf.Type) findMatch {
	match := findMatch{
		...
		Name:    name,
		Module:  module,
		BTFKind: btfx.TypeKind(typ),
	}
	if fn, ok := typ.(*btf.Func); ok {
		match.Prototype = btfx.FuncPrototype(fn)
	}
...
func addKernelFunctionMatch(module string, spec *btf.Spec, fn *btf.Func, ksyms *bpfsnoop.Kallsyms, collector *findCollector) {
	...
	match := findMatch{
		...
		Prototype: btfx.FuncPrototype(fn),
		BTF:       true,
...
func addKallsymsFunctionMatch(name string, info bpfsnoop.FindKernelFunctionInfo, collector *findCollector) {
	module := "vmlinux"
	if len(info.Symbols) != 0 {
		module = info.Symbols[0].Module
	}
	match := findMatch{
		Kind:            FindKindFunction,
		Name:            name,
		Module:          module,
		FunctionSymbols: make([]findKernelFunctionSymbol, len(info.Symbols)),
	}
...
func findKallsymsFunctions(ctx context.Context, pattern string, matcher glob.Glob, ksyms *bpfsnoop.Kallsyms, collector *findCollector) error {
	if glob.QuoteMeta(pattern) == pattern {
		info, ok := bpfsnoop.FindKernelSymbol(pattern, ksyms)
		if ok {
			addKallsymsFunctionMatch(pattern, info, collector)
		}
		return nil
	}

	for name := range bpfsnoop.FindKernelSymbolNames(ksyms) {
		if err := ctx.Err(); err != nil {
			return err
		}
		if matcher.Match(name) {
			info, _ := bpfsnoop.FindKernelSymbol(name, ksyms)
			addKallsymsFunctionMatch(name, info, collector)
		}
	}
	return nil
}
...
func populateKernelFunctionMetadata(ctx context.Context, result *findResult, ksyms *bpfsnoop.Kallsyms) error {
	...
	for i := range result.Matches {
		...
		var info bpfsnoop.FindKernelFunctionInfo
		if match.function != nil {
			info = bpfsnoop.FindKernelFunction(match.function, ksyms)
		} else {
			var ok bool
			info, ok = bpfsnoop.FindKernelSymbol(match.Name, ksyms)
			if !ok {
				continue
			}
		}
		...
		if match.function != nil && !info.Traceable {
			continue
		}
...
func findTracepoints(ctx context.Context, pattern string, ksyms *bpfsnoop.Kallsyms, collector *findCollector) error {
	...
	for name, tracepoint := range tracepoints {
		collector.add(findMatch{
			...
			Name:      name,
			Prototype: btfx.FuncPrototype(tracepoint.Func),
		})
	}
	return nil
}
	...
	if functions, err := info.FuncInfos(); err == nil {
		...
		for _, function := range functions {
			match.ProgramFunctions = append(match.ProgramFunctions, FindBPFProgramFunction{
				...
				Prototype:         btfx.FuncPrototype(function.Func),
			})
		}
	}
	...
	if options.IncludeXlatedInsns {
		...
		for iterator.Next() {
			...
			if fn := btf.FuncMetadata(instruction); fn != nil {
				...
				output.Prototype = btfx.FuncPrototype(fn)
			}
...
func findObjects(ctx context.Context, options FindOptions) (findResult, error) {
	...
	findSymbols := wantsFindKind(options.Kind, FindKindKsym)
	var ksyms *bpfsnoop.Kallsyms
	if findFunctions || findTracepoint || findSymbols {
	...
	if findSymbols {
		if err := findKallsymsSymbols(ctx, matcher, ksyms, collector); err != nil {
			return findResult{}, err
		}
	}
	...
	if findFunctions {
		if err := findKallsymsFunctions(ctx, options.Pattern, matcher, ksyms, collector); err != nil {
			return findResult{}, err
		}
	}
	if findTracepoint {
...
// findKallsymsSymbols includes data and other non-text symbols without changing
// the text-only indexes used by tracing and disassembly.
func findKallsymsSymbols(ctx context.Context, matcher glob.Glob, ksyms *bpfsnoop.Kallsyms, collector *findCollector) error {
	for symbol := range ksyms.Symbols() {
		if err := ctx.Err(); err != nil {
			return err
		}
		if !matcher.Match(symbol.Name()) {
			continue
		}
		...
		collector.add(findMatch{Kind: FindKindKsym, Name: symbol.Name(), Module: module, Address: symbol.Addr(), SymbolType: string(symbol.Type())})
	}
	return nil
}
