...
type findMatchOutput struct {
	Address        string                                `json:"address,omitempty" jsonschema:"exact hexadecimal address of a kallsyms symbol"`
	SymbolType     string                                `json:"symbol_type,omitempty" jsonschema:"symbol type letter from /proc/kallsyms"`
	Kind           string                                `json:"kind" jsonschema:"matched object kind: function, tracepoint, bpf_program, btf_type, or ksym"`
	...
	BTF            *bool                                 `json:"btf,omitempty" jsonschema:"whether the kernel function or loaded BPF program has BTF metadata"`
...
func Find(ctx context.Context, input FindOptions) (FindOutput, error) {
	...
	for i := range result.Matches {
		...
		if match.Kind == FindKindKsym {
			matchOutput.Address = fmt.Sprintf("%#x", match.Address)
			matchOutput.SymbolType = match.SymbolType
		}
