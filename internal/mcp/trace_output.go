...
type TraceOutput struct {
	Action     string                       `json:"action,omitempty" jsonschema:"trace lifecycle action that produced this response"`
	State      string                       `json:"state,omitempty" jsonschema:"ready once start has attached the trace"`
	ReadyAt    string                       `json:"ready_at,omitempty" jsonschema:"UTC time when tracing became ready"`
	Deadline   string                       `json:"deadline,omitempty" jsonschema:"UTC time when tracing will stop by duration"`
	Aborted    bool                         `json:"aborted,omitempty" jsonschema:"true when abort cancelled the active trace"`
...
func makeTraceSelectedArgumentsOutput(arguments []bpfsnoop.TraceSelectedArgument) ([]traceSelectedArgumentOutput, error) {
	if len(arguments) == 0 {
		return nil, nil
	}
	output := make([]traceSelectedArgumentOutput, 0, len(arguments))
	for _, argument := range arguments {
		value, err := kernelReadValue(&argument.Raw)
		if err != nil {
			return nil, fmt.Errorf("failed to decode selected argument %q: %w", argument.Raw.Expression, err)
		}
		output = append(output, traceSelectedArgumentOutput{
			Expression: argument.Raw.Expression,
			Type:       argument.Raw.Type,
			Value:      value,
		})
	}
	return output, nil
...
func makeTraceEventOutput(event bpfsnoop.TraceEvent) (traceEventOutput, error) {
	selectedArguments, err := makeTraceSelectedArgumentsOutput(event.SelectedArguments)
	if err != nil {
		return traceEventOutput{}, err
	}
